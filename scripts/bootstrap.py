...
CONFIG_REPO_PIN_ENV = "ZEN70_CONFIG_REPO_PINNED_COMMIT"
ALLOW_UNPINNED_CONFIG_REPO_ENV = "ZEN70_ALLOW_UNPINNED_CONFIG_REPO"
...
def _allow_unpinned_config_repo() -> bool:
    return os.getenv(ALLOW_UNPINNED_CONFIG_REPO_ENV, "").strip().lower() in {"1", "true", "yes", "on"}


def _configured_config_repo_commit() -> str | None:
    raw_value = os.getenv(CONFIG_REPO_PIN_ENV, "").strip().lower()
    if not raw_value:
        return None
    if not _GIT_COMMIT_RE.fullmatch(raw_value):
        raise RuntimeError(f"{CONFIG_REPO_PIN_ENV} must be a full 40-character commit SHA")
    return raw_value


def _enforce_config_repo_pin_policy() -> str | None:
    pinned_commit = _configured_config_repo_commit()
    if pinned_commit is not None:
        return pinned_commit
    if _is_production_env() and not _allow_unpinned_config_repo():
        raise RuntimeError(
            f"{CONFIG_REPO_PIN_ENV} is required in production bootstrap runs "
            f"(or set {ALLOW_UNPINNED_CONFIG_REPO_ENV}=1 for an explicit break-glass override)"
        )
    if logger:
        logger.warning("Config repository is not commit-pinned; bootstrap will trust the remote main branch")
    return None
...
def pull_latest_config(root: Path, config_path: Path, offline: bool = False) -> bool:
    ...
    try:
        pinned_commit = _enforce_config_repo_pin_policy()
    except RuntimeError as exc:
        if logger:
            logger.error("Configuration source policy rejected bootstrap: %s", exc)
        return False

    repo_path = root / REPO_DIR_NAME
    if _pull_from_remote_sources(root, repo_path, config_path, pinned_commit):
        return True
...
def _sync_repo_from_source(root: Path, repo_path: Path, source: str, pinned_commit: str | None) -> bool:
    """克隆或更新配置仓库，成功返回 True。"""
    if repo_path.exists():
        result = _run_git(
            root,
            ["git", "-C", str(repo_path), "remote", "set-url", "origin", source],
            timeout=30,
        )
        if result.returncode != 0:
            if logger:
                logger.warning("git remote set-url 失败: %s", result.stderr or result.stdout)
            return False
    else:
        clone_args = ["git", "clone"]
        if pinned_commit is None:
            clone_args.extend(["--depth", "1", "--single-branch", "--branch", "main"])
        else:
            clone_args.append("--no-checkout")
        clone_args.extend([source, str(repo_path)])
        result = _run_git(root, clone_args, timeout=120)
        if result.returncode != 0:
            if logger:
                logger.warning("git clone 失败: %s", result.stderr or result.stdout)
            return False

    fetch_target = pinned_commit or "main"
    result = _run_git(
        root,
        ["git", "-C", str(repo_path), "fetch", "--depth", "1", "origin", fetch_target],
        timeout=120,
    )
    if result.returncode != 0:
        if logger:
            logger.warning("git fetch 失败: %s", result.stderr or result.stdout)
        return False
    ...
    if result.returncode != 0:
        if logger:
            logger.warning("git checkout 失败: %s", result.stderr or result.stdout)
        return False
    ...
    if head_result.returncode != 0:
        if logger:
            logger.warning("git rev-parse 失败: %s", head_result.stderr or head_result.stdout)
        return False
    current_head = head_result.stdout.strip().lower()
    if pinned_commit is not None and current_head != pinned_commit:
        if logger:
            logger.warning("配置仓库 commit 不匹配: expected=%s actual=%s", pinned_commit, current_head)
        return False
    return True


def _sync_repo_from_source(root: Path, repo_path: Path, source: str, pinned_commit: str | None) -> bool:
    """Synchronize the cached config repository from a specific remote source."""
    if repo_path.exists():
        result = _run_git(
            root,
            ["git", "-C", str(repo_path), "remote", "set-url", "origin", source],
        ...
        if result.returncode != 0:
            if logger:
                logger.warning("git remote set-url failed: %s", result.stderr or result.stdout)
            return False
    else:
        clone_args = ["git", "clone"]
        if pinned_commit is None:
        ...
        if result.returncode != 0:
            if logger:
                logger.warning("git clone failed: %s", result.stderr or result.stdout)
            return False
    ...
    if result.returncode != 0:
        if logger:
            logger.warning("git fetch failed: %s", result.stderr or result.stdout)
        return False
    ...
    if result.returncode != 0:
        if logger:
            logger.warning("git checkout failed: %s", result.stderr or result.stdout)
        return False
    ...
    if head_result.returncode != 0:
        if logger:
            logger.warning("git rev-parse failed: %s", head_result.stderr or head_result.stdout)
        return False
    current_head = head_result.stdout.strip().lower()
    if pinned_commit is not None and current_head != pinned_commit:
        if logger:
            logger.warning("Pinned config repo commit mismatch: expected %s got %s", pinned_commit, current_head)
        return False
    return True
...
def _pull_from_remote_sources(root: Path, repo_path: Path, config_path: Path, pinned_commit: str | None) -> bool:
    for source in GIT_SOURCES:
        try:
            if logger:
                logger.info("灏濊瘯鎷夊彇閰嶇疆: %s", source)
            if not _sync_repo_from_source(root, repo_path, source, pinned_commit):
                continue
            candidate = repo_path / DEFAULT_CONFIG_NAME
            if _copy_verified_candidate(repo_path, candidate, config_path):
                return True
            if logger:
                logger.warning("?????? %s???????", DEFAULT_CONFIG_NAME)
        except subprocess.TimeoutExpired:
            if logger:
                logger.warning("鎷夊彇瓒呮椂: %s", source)
        except (OSError, ValueError, KeyError, RuntimeError, TypeError) as exc:
            if logger:
                logger.warning("鎷夊彇寮傚父 %s: %s", source, exc, exc_info=True)
    return False
