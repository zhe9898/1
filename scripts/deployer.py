...
def _start_host_services(config_path: Path, systemd_dir: Path) -> None:
    """为 system.yaml 中 runtime: host 的服务执行 systemctl daemon-reload + enable --now。

    仅在 Linux 系统上执行；unit 文件需已由 compiler.py 写入 systemd_dir。
    """
    ...
    # 将生成的 unit 文件链接/复制到 /etc/systemd/system
