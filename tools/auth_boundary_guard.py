...
def auth_boundary_violations(*, repo_root: Path | None = None) -> list[str]:
    ...
    for path in sorted(backend_root.rglob("*.py")):
        ...
        if rel.startswith("backend/tests/"):
            continue
        if rel in DIRECT_ROLE_CLAIM_ALLOWLIST:
            continue
        tree = ast.parse(path.read_text(encoding="utf-8-sig"), filename=str(path))
        for node in ast.walk(tree):
            if isinstance(node, ast.Call) and _is_current_user_role_get(node):
                violations.append(f"{rel}:{getattr(node, 'lineno', 0)}:direct current_user role claim access")
            elif isinstance(node, ast.Subscript) and _is_current_user_role_subscript(node):
                violations.append(f"{rel}:{getattr(node, 'lineno', 0)}:direct current_user role claim access")
    return violations
