...
from backend.core.webhooks import post_public_webhook_async
...
async def _system_guardian_emit_critical_alert(self: SystemGuardian, title: str, message: str) -> None:
    if not self.alert_webhook:
        logger.warning("ALERT_WEBHOOK_URL 鏈厤缃紝璺宠繃鍛婅涓嬪彂: %s", title)
        return
    sent = await post_public_webhook_async(
        self.alert_webhook,
        {
