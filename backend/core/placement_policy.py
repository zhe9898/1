"""Placement Policy Framework — pluggable system-level placement solver.

Addresses the gap between per-job scheduling strategies (SPREAD/BINPACK/…)
and a true system-level placement optimiser.  The ``PlacementPolicy``
protocol defines three extension points:

1. ``adjust_score``   — modify per-(job, node) scores before ranking.
2. ``rerank``         — global reranking after scoring (e.g. bin-pack
                        consolidation or topology-aware grouping).
3. ``accept``         — final veto gate (resource reservation, anti-
                        starvation cap, etc.).
