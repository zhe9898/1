...
from backend.core.auth_helpers import CODE_BAD_REQUEST, CODE_SERVER_ERROR, CODE_UNAUTHORIZED, get_challenge_from_credential
from backend.core.constants import KEY_AUTH_CHALLENGE_PREFIX
...
class WebAuthnChallengeStore:
    ...
    async def _cache_snapshot(
        ...
        *,
        ...
        try:
            await redis.set_auth_challenge(challenge.challenge_id, payload, ttl=ttl_seconds)
        except Exception:
            return
    ...
    async def _clear_cache(redis: RedisClient | None, challenge_id: str) -> None:
        if redis is None:
            return
        try:
            await redis.delete(f"{KEY_AUTH_CHALLENGE_PREFIX}{challenge_id}")
        except Exception:
            return
