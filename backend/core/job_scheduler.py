    ...
    def _greedy_match(
        ...
        while heap:
            ...
            if remaining_cap.get(node_id, 0) <= 0:
                if gid:
                    # Gang member can't be placed → fail entire gang
                    # Rollback any tentative assignments for this gang
                    self._rollback_gang(gid, gang_tentative, gang_cap_deltas, remaining_cap)
                    failed_gangs.add(gid)
                    logger.debug("gang_placement_failed: gang=%s member=%s no_capacity=%s", gid, job_id, node_id)
                continue
