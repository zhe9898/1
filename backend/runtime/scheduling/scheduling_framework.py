"""Pluggable Scheduling Framework 鈥?phase-based extensible pipeline.
...
1. **QueueSort**   鈥?custom ordering of the candidate queue.
2. **PreFilter**   鈥?lightweight pre-admission (e.g., quota check).
3. **Filter**      鈥?hard feasibility gates (resource fit, affinity鈥?.
4. **PostFilter**  鈥?after filtering 鈥?e.g., log dropped candidates.
5. **Score**       鈥?soft scoring modifiers (fair-share, priority boost).
6. **Reserve**     鈥?tentatively claim resources for accepted jobs.
7. **Permit**      鈥?final admission control before lease (gang wait).
8. **PreBind**     鈥?pre-binding hooks (e.g., create volumes).
9. **Bind**        鈥?actual lease/assignment.
10. **PostBind**   鈥?post-binding side effects (audit, metrics).
...
- Nomad scheduler extensibility: eval 鈫?plan 鈫?apply
...
from backend.extensions.extension_guard import validate_scheduling_profile_budget
...
class PluginStatus(Enum):
    ...
    SUCCESS = "success"
    REJECT = "reject"  # Hard rejection 鈥?job is dropped
    SKIP = "skip"  # Plugin has nothing to do for this job
    WAIT = "wait"  # Permit phase: hold the job (gang scheduling)
    ERROR = "error"  # Plugin errored 鈥?treatment depends on tolerance
...
class FilterPlugin(ABC):
    """Hard feasibility gate 鈥?per-job evaluation."""
...
class ScorePlugin(ABC):
    """Soft scoring modifier 鈥?per-job evaluation."""
...
class PermitPlugin(ABC):
    """Final admission control 鈥?can WAIT (gang) or REJECT."""
...
class PreBindPlugin(ABC):
    """Pre-binding hook 鈥?called after permit, before lease."""
...
# =====================================================================
# Scheduling Profile 鈥?plugin registry per scheduling profile
# =====================================================================
...
# =====================================================================
# Scheduling Pipeline 鈥?executes plugins through phases
# =====================================================================
...
class SchedulingPipeline:
    ...
    The pipeline is stateless 鈥?create one per dispatch cycle with the
    appropriate profile.
    ...
    def run_queue_sort(self, candidates: list[Job]) -> list[Job]:
        """Phase 1: QueueSort 鈥?reorder candidates."""
    ...
    def run_pre_filter(self, ctx: SchedulingContext) -> PluginResult:
        """Phase 2: PreFilter 鈥?cycle-level admission."""
    ...
    def run_filter(
        ...
        """Phase 3: Filter 鈥?per-job hard gates.
    ...
    def run_post_filter(
        ...
        """Phase 4: PostFilter 鈥?observability / fallback."""
    ...
    def run_score(
        ...
        """Phase 5: Score 鈥?collect score deltas per job.
    ...
    def run_reserve(self, job: Job, ctx: SchedulingContext) -> PluginResult:
        """Phase 6: Reserve 鈥?claim resources."""
    ...
    def run_permit(self, job: Job, ctx: SchedulingContext) -> PluginResult:
        """Phase 7: Permit 鈥?final admission (may WAIT for gang)."""
    ...
    def run_pre_bind(self, job: Job, ctx: SchedulingContext) -> PluginResult:
        """Phase 8: PreBind 鈥?prepare before lease."""
    ...
    def run_bind(self, job: Job, ctx: SchedulingContext) -> PluginResult:
        """Phase 9: Bind 鈥?perform lease assignment."""
    ...
    def run_full(
        ...
        This is the canonical filter+score execution path for profile-driven scheduling.
...
# =====================================================================
# Profile Registry 鈥?named profiles for different workload classes
# =====================================================================
...
# =====================================================================
# Built-in adapter: bridge existing SchedulingConstraint 鈫?FilterPlugin
# =====================================================================


class ConstraintFilterAdapter(FilterPlugin):
    """Project a SchedulingConstraint into the FilterPlugin interface.

    This keeps the authoritative constraint set reusable inside the
    scheduling profile pipeline.
    """

    def __init__(self, constraint: object) -> None:
        from backend.runtime.scheduling.scheduling_constraints import SchedulingConstraint as SC
...
def build_profile_from_engine() -> SchedulingProfile:
    """Build the default SchedulingProfile from the authoritative SchedulingEngine.

    Constraint objects are projected into the phase-oriented plugin model
    so both execution paths consume the same gating logic.
    """
    from backend.runtime.scheduling.scheduling_constraints import get_scheduling_engine
