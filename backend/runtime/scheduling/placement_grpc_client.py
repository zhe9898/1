...
``async_solve(jobs, nodes, *, now, accepted_kinds, ...) -> dict[str, str]``
    Async planner entrypoint that preserves the solver result contract.

``async_build_time_budgeted_placement_plan(...)``
    Async planner entrypoint that preserves the time-budgeted planning contract.
"""
...
def _get_grpc_stub() -> object | None:
    ...
    try:
        ...
        from backend.runtime.scheduling.gen_grpc import placement_pb2_grpc
...
def _job_to_proto(job: Job) -> object:
    ...
    from backend.runtime.scheduling.gen_grpc import placement_pb2
...
async def async_solve(
    ...
    *,
    ...
    """Run placement planning asynchronously without blocking the event loop.
    ...
    from backend.runtime.scheduling.placement_solver import get_placement_solver
...
async def async_build_time_budgeted_placement_plan(
    ...
    *,
    ...
    """Run time-budgeted placement planning asynchronously.
    ...
    from backend.runtime.scheduling.placement_solver import _get_solver_config
