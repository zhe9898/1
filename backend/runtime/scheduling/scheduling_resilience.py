"""Scheduling Resilience 鈥?industry-standard scheduling gaps filled.
...
1. **TopologySpreadPolicy** 鈥?zone-aware spreading with maxSkew
   (K8s TopologySpreadConstraints equivalent).
2. **PreemptionBudgetPolicy** 鈥?disruption budget limiting concurrent
   preemptions (K8s PodDisruptionBudget equivalent).
3. **SchedulingBackoff** 鈥?exponential retry backoff for unschedulable
   jobs (K8s unschedulable-backoff equivalent).
4. **AdmissionController** 鈥?queue depth backpressure rejecting
   submissions when tenant queue is saturated (K8s ResourceQuota /
   Nomad job-quota equivalent).
5. **SchedulingMetrics** 鈥?lightweight in-memory throughput, latency,
...
if TYPE_CHECKING:
    ...
    from backend.runtime.scheduling.job_scheduler import SchedulerNodeSnapshot, ScoredJob
...
class SchedulingBackoff:
    ...
    back-off (1s 鈫?2s 鈫?4s 鈥?鈫?5min cap).
    """
    ...
    def _cleanup(cls, now: datetime.datetime) -> None:
        """Remove entries whose backoff has long expired (>2脳 max_delay)."""
        with cls._lock:
...
class AdmissionController:
    """Queue depth backpressure 鈥?reject new submissions when saturated.
...
class SchedulingMetrics:
    ...
    Not a replacement for Prometheus 鈥?this provides an instant snapshot
    ...
    @classmethod
    def record_tuner_failure(cls) -> None:
        cls._tuner_failures += 1
    ...
    def snapshot(cls, window_seconds: float = 300) -> dict[str, object]:
        ...
        return {
            ...
            "tuner_failures_total": cls._tuner_failures,
        }
