...
class DependencyGate(SchedulingConstraint):
    ...
    def evaluate(self, job: Job, ctx: SchedulingContext) -> tuple[bool, str]:
        from backend.runtime.scheduling.business_scheduling import check_job_dependencies_satisfied
...
class GangSchedulingGate(SchedulingConstraint):
    ...
    - ``"fail"``    鈥?drop the gang (all members rejected).
    - ``"degrade"`` 鈥?release members for independent scheduling.
    """
    ...
    def evaluate(self, job: Job, ctx: SchedulingContext) -> tuple[bool, str]:
        from backend.runtime.scheduling.gang_scheduler import calculate_gang_scheduling_readiness

        gang_id = getattr(job, "gang_id", None)
        if not gang_id:
            return True, ""

        # 鈹€鈹€ Gang timeout check 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
        try:
        ...
        if wait_timeout_s > 0:
            ...
            if age_s > wait_timeout_s:
                if timeout_action == "degrade":
                    # Release for independent scheduling 鈥?pass the gate
...
class PriorityBoostModifier(SchedulingConstraint):
    ...
    def evaluate(self, job: Job, ctx: SchedulingContext) -> tuple[bool, str]:
        from backend.runtime.scheduling.business_scheduling import calculate_boosted_priority
...
class TenantFairShareGate(SchedulingConstraint):
    ...
    def evaluate(self, job: Job, ctx: SchedulingContext) -> tuple[bool, str]:
        from backend.runtime.scheduling.queue_stratification import get_fair_scheduler
...
def _build_default_engine() -> SchedulingEngine:
    ...
    # errors if the modules are not yet present 鈥?defensive coding).
    try:
        from backend.runtime.scheduling.quota_aware_scheduling import (
        ...
        pass  # Module not available 鈥?skip
