...
from backend.control_plane.adapters.auth_shared import assert_user_active, register_login_session, request_tenant_id
from backend.control_plane.adapters.auth_token_issue import issue_auth_token
from backend.control_plane.adapters.deps import get_db, get_redis
from backend.control_plane.adapters.models.auth import AuthSessionResponse, PasswordLoginRequest
...
async def password_login(
    ...
    from backend.control_plane.adapters.auth_shared import first_user_or_schema_unavailable
    ...
    if not is_valid:
        ...
        if new_count >= 5:
            ...
            raise zen(CODE_TOO_MANY, "Too many failed attempts; IP locked for 15 minutes", status.HTTP_429_TOO_MANY_REQUESTS)
