...
from backend.control_plane.adapters.models.auth import AuthSessionResponse, BootstrapRequest
...
async def sys_status(db: AsyncSession | None = Depends(get_db)) -> dict[str, bool]:
    """Report whether the system has already been bootstrapped."""
    if db is None:
...
async def bootstrap(
    ...
    """Create the first admin account when the system is still empty."""
