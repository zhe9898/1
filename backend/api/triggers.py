...
async def upsert_trigger(
    ...
    if trigger is None:
        ...
        trigger.config = validated_config
        trigger.input_defaults = dict(payload.input_defaults)
        trigger.target = validated_target
    ...
    if webhook_config.secret:
        ...
        if not hmac.compare_digest(str(provided_secret), str(webhook_config.secret)):
            raise zen(
                ...
                status_code=401,
                recovery_hint="Provide the configured webhook secret header and retry",
                details={"trigger_id": trigger_id, "secret_header": webhook_config.secret_header},
            )
