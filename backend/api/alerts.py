...
def _alert_to_response(a: Alert) -> AlertResponse:
    return AlertResponse(
        id=a.id, rule_id=a.rule_id, rule_name=a.rule_name,
        severity=a.severity, message=a.message, details=a.details,
        notified=a.notified, triggered_at=a.triggered_at.isoformat(),
...
async def update_alert_rule(
    ...
    result = await db.execute(
        select(AlertRule).where(AlertRule.id == rule_id, AlertRule.tenant_id == current_user["tenant_id"])
    )
