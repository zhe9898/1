...
注意：成功响应 Envelope 包装 (ADR 0010) 的唯一权威实现在 backend/control_plane/app/response_envelope.py。
...
_READONLY_REDIS_TIMEOUT_SECONDS = float(os.getenv("READONLY_REDIS_TIMEOUT_SECONDS", "2.0"))
...
async def global_readonly_lock(request: Request, call_next: RequestResponseEndpoint) -> JSONResponse | Response:
    ...
    if request.method in ("POST", "PUT", "DELETE", "PATCH"):
        ...
        if matrix is None:
            ...
            except (asyncio.TimeoutError, OSError, ValueError, KeyError, RuntimeError, TypeError):
    ...
    for svc_name, is_ready in tuple(dict(service_readiness).items()):
        if not is_ready:
