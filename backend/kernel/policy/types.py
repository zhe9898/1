"""Kernel scheduling policy data structures.

These immutable dataclasses define the canonical kernel-owned scheduling
policy contract. Validation lives in ``backend.kernel.policy.validation`` and
runtime state lives in ``backend.kernel.policy.store_runtime``.
"""
...
class ScoringWeights:
    """Per-dimension scoring caps before auto-tune multipliers."""
...
@dataclass(frozen=True, slots=True)
class RetryPolicy:
    """Retry and backoff parameters for job recovery."""
...
class BalancedWeights:
    """Balanced strategy blend weights."""
...
class DispatchConfig:
    """Job dispatch and pull query parameters."""
...
class BackfillPolicyConfig:
    """Backfill and reservation scheduling parameters."""
...
class SchedulingPolicy:
    """The complete set of externalizable kernel scheduling parameters."""
    ...
    resource_reservation: ResourceReservationConfig = field(default_factory=ResourceReservationConfig)
    strategy: StrategyConfig = field(default_factory=StrategyConfig)
    queue: QueueConfig = field(default_factory=QueueConfig)
    service_classes: dict[str, ServiceClassDef] = field(default_factory=lambda: dict(_DEFAULT_SERVICE_CLASSES))
