"""Scheduling Governance 鈥?DB-first tenant policy + decision audit logger.
...
- ``get_tenant_policy`` / ``upsert_tenant_policy`` 鈥?CRUD for per-tenant
  scheduling policies stored in ``TenantSchedulingPolicy``.
- ``SchedulingDecisionLogger`` 鈥?accumulates placed/rejected/preempted job
  records during a dispatch cycle and flushes them to ``SchedulingDecision``.
- ``is_scheduling_feature_enabled`` / ``set_scheduling_feature`` 鈥?DB-backed
...
class SchedulingDecisionLogger:
    ...
    async def flush(self, db: AsyncSession) -> SchedulingDecision | None:
        ...
        Returns the decision row (not yet committed 鈥?caller owns the tx).
...
# Default states: conservative 鈥?all new capabilities start disabled
