"""Quota-Aware Fair-Share Scheduling 鈥?kernel-level resource quota enforcement.
...
Capabilities:
1. **ResourceQuotaAccount** 鈥?per-tenant resource accounting (CPU, memory,
   GPU, concurrent jobs) tracked during each dispatch cycle.
2. **FairShareCalculator** 鈥?DRF (Dominant Resource Fairness) inspired
   allocation that considers *resource usage* not just job count.
3. **QuotaAwareGate** 鈥?SchedulingConstraint that rejects jobs whose
   tenant has exceeded any resource dimension quota.
4. **FairShareScoreModifier** 鈥?soft constraint that penalises tenants
...
# =====================================================================
# 1. Resource Quota Account 鈥?per-tenant resource tracking
# =====================================================================
...
# =====================================================================
# 2. Fair-Share Calculator 鈥?DRF-inspired allocation
# =====================================================================
...
class FairShareCalculator:
    ...
    - < 1.0 鈫?tenant is under-served (should be boosted)
    - = 1.0 鈫?tenant is at fair share
    - > 1.0 鈫?tenant is over-served (should be penalised)
    """
...
# =====================================================================
# 3. QuotaAwareGate 鈥?hard constraint in the scheduling pipeline
# =====================================================================
...
class QuotaAwareGate(SchedulingConstraint):
    ...
    name = "resource_quota"
    order = 5  # Very early 鈥?before fair-share, deps, gangs
    hard = True
...
# =====================================================================
# 4. FairShareScoreModifier 鈥?soft penalty/boost based on fair-share
# =====================================================================


# Maximum score adjustment (positive or negative) 鈥?read from policy store
...
class FairShareScoreModifier(SchedulingConstraint):
    ...
    Score adjustment = clamp(-40, +40, -40 脳 (ratio - 1.0))
    - ratio < 1.0 鈫?positive boost (under-served)
    - ratio = 1.0 鈫?no change
    - ratio > 1.0 鈫?negative penalty (over-served)
    """
...
def load_resource_quotas() -> dict[str, ResourceQuotaLimit]:
    ...
    at startup; this function reads from there 鈥?never from the file directly.
