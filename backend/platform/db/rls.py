...
_PG_IDENTIFIER_RE: Final[re.Pattern[str]] = re.compile(r"^[a-z_][a-z0-9_]*$")
...
def _quote_pg_identifier(identifier: str) -> str:
    if not _PG_IDENTIFIER_RE.fullmatch(identifier):
        raise RuntimeError(f"Unsafe PostgreSQL identifier: {identifier}")
    return f'"{identifier}"'
...
async def apply_rls_policies(session: AsyncSession) -> None:
    ...
    for table_name in _TENANT_TABLES:
        try:
            ...
            quoted_table_name = _quote_pg_identifier(table_name)
            await session.execute(text(f"ALTER TABLE {quoted_table_name} ENABLE ROW LEVEL SECURITY;"))
            await session.execute(text(f"ALTER TABLE {quoted_table_name} FORCE ROW LEVEL SECURITY;"))

            policy_name = f"{_POLICY_PREFIX}_{table_name}"
            quoted_policy_name = _quote_pg_identifier(policy_name)
            await session.execute(text(f"DROP POLICY IF EXISTS {quoted_policy_name} ON {quoted_table_name};"))
            await session.execute(
                text(
                    f"CREATE POLICY {quoted_policy_name} ON {quoted_table_name} "
