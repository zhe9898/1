...
class TestEvaluationCreateRequest:
    ...
    def test_valid_payload(self) -> None:
        from backend.api.evaluations_helpers import EvaluationCreateRequest
    ...
    def test_all_valid_categories_accepted(self) -> None:
        from backend.api.evaluations_helpers import VALID_CATEGORIES, EvaluationCreateRequest
...
class TestEvaluationStatusView:
    ...
    def test_submitted_label(self) -> None:
        from backend.api.evaluations_helpers import _evaluation_status_view
...
class TestBuildEvaluationActions:
    ...
    def test_submitted_can_delete(self) -> None:
        from backend.api.evaluations_helpers import _build_evaluation_actions
...
class TestToResponse:
    ...
    def test_fields_mapped(self) -> None:
        from backend.api.evaluations_helpers import _to_response
...
class TestResourceSchema:
    ...
    def test_resource_is_evaluation(self) -> None:
        from backend.api.evaluations_helpers import _resource_schema
...
class TestEvalStmtForTenant:
    ...
    def test_returns_select(self) -> None:
        ...
        from backend.api.evaluations import _eval_stmt_for_tenant
...
class TestUtcnow:
    ...
    def test_returns_datetime(self) -> None:
        from backend.api.evaluations import _utcnow
...
class TestCreateEvaluation:
    ...
    async def test_create_success(self) -> None:
        from backend.api.evaluations import create_evaluation
        from backend.api.evaluations_helpers import EvaluationCreateRequest
        ...
        with patch("backend.api.evaluations._to_response") as mock_resp:
    ...
    @pytest.mark.asyncio
    async def test_tenant_id_defaults_to_default(self) -> None:
        """current_user 无 tenant_id 时使用 'default'。"""
        from backend.api.evaluations import create_evaluation
        from backend.api.evaluations_helpers import EvaluationCreateRequest
        ...
        created_ev = _make_eval(evaluation_id="ex", tenant_id="default")
        ...
        with patch("backend.api.evaluations._to_response") as mock_resp:
            mock_resp.return_value = MagicMock(evaluation_id="ex")
            await create_evaluation(payload=payload, current_user=user, db=db)

        # The object added to db should have tenant_id == "default"
        assert len(added_obj) == 1
        assert added_obj[0].tenant_id == "default"  # type: ignore[union-attr]
...
class TestListEvaluations:
    ...
    async def test_list_no_filters_returns_all(self) -> None:
        from backend.api.evaluations import list_evaluations
        ...
        with patch("backend.api.evaluations._to_response", side_effect=lambda e: e):
...
class TestGetEvaluation:
    ...
    async def test_found_returns_response(self) -> None:
        from backend.api.evaluations import get_evaluation
        ...
        with patch("backend.api.evaluations._to_response", return_value=MagicMock(evaluation_id="eval-001")):
...
class TestDeleteEvaluation:
    ...
    async def test_delete_submitted_success(self) -> None:
        from backend.api.evaluations import delete_evaluation
...
class TestGetEvaluationSchema:
    ...
    async def test_schema_endpoint_returns_resource_schema(self) -> None:
        from backend.api.evaluations import get_evaluation_schema
