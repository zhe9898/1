...
from backend.control_plane.adapters.jobs import JobCreateRequest, JobPullRequest, JobResultRequest, complete_job, pull_jobs
from backend.control_plane.adapters.jobs.submission_service import check_concurrent_limits, submit_job
...
async def test_submit_job_returns_503_when_transaction_recovery_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("backend.control_plane.adapters.jobs.submission_service.validate_job_payload", lambda *_: {"ok": True})
    monkeypatch.setattr("backend.control_plane.adapters.jobs.submission_service.resolve_job_queue_contract", lambda **_: ("interactive", "default"))
    ...
    with pytest.raises(HTTPException) as exc:
        await submit_job(
            ...
            current_user={"sub": "u1", "tenant_id": "default", "role": "admin"},
            db=db,
            redis=None,
        )
...
async def test_complete_job_commits_before_publishing_event(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("backend.control_plane.adapters.jobs.lifecycle_service.get_reservation_manager", lambda: reservation_mgr)
    ...
    monkeypatch.setattr("backend.control_plane.adapters.jobs.lifecycle_service.publish_control_event", _publish)
...
async def test_pull_jobs_commits_then_publishes_then_releases_lock(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    redis = MagicMock()
    redis.locks = MagicMock()
    redis.locks.acquire = AsyncMock(return_value=True)
    redis.locks.release = AsyncMock(side_effect=lambda _: order.append("release"))

    async def _publish(*args: object, **kwargs: object) -> None:
        del args, kwargs
        order.append("publish")

    monkeypatch.setattr("backend.control_plane.adapters.jobs.pull_service.publish_control_event", _publish)
    ...
    monkeypatch.setattr("backend.control_plane.adapters.jobs.pull_service.async_build_time_budgeted_placement_plan", AsyncMock(return_value={}))
...
async def test_pull_jobs_releases_lock_on_exception(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    redis.locks = MagicMock()
    redis.locks.acquire = AsyncMock(return_value=True)
    redis.locks.release = AsyncMock(return_value=True)
    ...
    redis.locks.release.assert_awaited_once()
