...
class TestFairSchedulerProxies:
    def test_get_tenant_quota(self, facade: GovernanceFacade) -> None:
        with patch(
            "backend.core.queue_stratification.get_fair_scheduler"
        ) as mock_get:
            fs = MagicMock()
...
class TestPlacementSolverProxy:
    def test_run_placement_solver(self, facade: GovernanceFacade) -> None:
        with patch(
            "backend.core.job_scheduler.get_placement_solver"
        ) as mock_get:
            solver = MagicMock()
            ...
            result = facade.run_placement_solver(
                jobs=["j1"], nodes=["n1"],
                now=_now(), accepted_kinds={"shell.exec"},
            )
        assert result == {"j1": "n1"}
...
class TestExecutorRegistryProxies:
    def test_validate_node_executor(self, facade: GovernanceFacade) -> None:
        with patch(
            "backend.core.executor_registry.get_executor_registry"
        ) as mock_get:
            reg = MagicMock()
...
class TestDispatchLifecycleProxy:
    def test_get_dispatch_pipeline(self, facade: GovernanceFacade) -> None:
        with patch(
            "backend.core.dispatch_lifecycle.get_dispatch_pipeline"
        ) as mock_get:
            pipeline = MagicMock()
