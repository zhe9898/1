...
class TestFailureControlPlaneProxies:
    ...
    async def test_is_node_quarantined(self, facade: GovernanceFacade) -> None:
        with patch("backend.runtime.scheduling.failure_control_plane.get_failure_control_plane") as mock_get:
            fcp = MagicMock()
...
class TestFairSchedulerProxies:
    def test_get_tenant_quota(self, facade: GovernanceFacade) -> None:
        with patch("backend.runtime.scheduling.queue_stratification.get_fair_scheduler") as mock_get:
            fs = MagicMock()
...
class TestPlacementSolverProxy:
    def test_run_placement_solver(self, facade: GovernanceFacade) -> None:
        with patch("backend.runtime.scheduling.job_scheduler.get_placement_solver") as mock_get:
            solver = MagicMock()
...
class TestExecutorRegistryProxies:
    def test_validate_node_executor(self, facade: GovernanceFacade) -> None:
        with patch("backend.runtime.topology.executor_registry.get_executor_registry") as mock_get:
            reg = MagicMock()
...
class TestDispatchLifecycleProxy:
    def test_get_dispatch_pipeline(self, facade: GovernanceFacade) -> None:
        with patch("backend.runtime.execution.dispatch_lifecycle.get_dispatch_pipeline") as mock_get:
            pipeline = MagicMock()
