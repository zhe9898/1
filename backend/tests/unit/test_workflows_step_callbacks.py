...
async def test_report_step_complete_rejects_job_mismatch(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    db.execute.side_effect = [scalar_result(workflow), scalar_result(step), scalar_result(job)]

    monkeypatch.setattr(
        "backend.control_plane.adapters.workflow_machine_callbacks.authenticate_node_request",
        AsyncMock(return_value=SimpleNamespace(node_id="node-a")),
    )
