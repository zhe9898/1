...
async def test_handle_voice_retry_failure_ignores_missing_message_id() -> None:
    ...
    redis.redis.incr.assert_not_awaited()
    redis.redis.xadd.assert_not_awaited()
...
async def test_handle_voice_retry_failure_moves_message_to_dlq_after_threshold() -> None:
    ...
    redis.redis.incr.assert_awaited_once_with("zen70:voice:retries:1-0")
    redis.redis.expire.assert_awaited_once_with("zen70:voice:retries:1-0", 3600)
    redis.redis.xadd.assert_awaited_once()
    xadd_args, xadd_kwargs = redis.redis.xadd.await_args
    assert xadd_args[0] == VOICE_DLQ_STREAM
    assert xadd_args[1]["msg_id"] == "1-0"
    redis.redis.xack.assert_awaited_once_with(VOICE_INPUT_STREAM, CONSUMER_GROUP, "1-0")
    redis.redis.delete.assert_awaited_once_with("zen70:voice:retries:1-0")
