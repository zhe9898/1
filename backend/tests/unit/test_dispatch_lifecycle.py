...
from backend.runtime.execution.dispatch_lifecycle import (
    AdmissionStage,
    DispatchContext,
    DispatchPipeline,
    DispatchResult,
...
class TestGovernanceStages:
    ...
    async def test_filtering_stage_executor_validation_fails_closed_without_executor(
        ...
        monkeypatch.setattr(
            "backend.runtime.scheduling.failure_control_plane.get_failure_control_plane",
            lambda: MagicMock(
        ...
        monkeypatch.setattr(
            "backend.runtime.scheduling.governance_facade.get_governance_facade",
            lambda: MagicMock(
