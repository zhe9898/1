...
class TestJwtDualTrack:
    ...
    async def test_create_and_decode_current_secret(self) -> None:
        ...
        payload, new_token = await self.jwt_mod.decode_token(token, redis_conn=_redis_allowing_tokens())
    ...
    async def test_new_token_from_previous_is_valid(self) -> None:
        ...
        _, new_token = await self.jwt_mod.decode_token(old_token, redis_conn=_redis_allowing_tokens())
        assert new_token is not None

        payload2, new_token2 = await self.jwt_mod.decode_token(new_token, redis_conn=_redis_allowing_tokens())
    ...
    async def test_half_lifespan_auto_renew(self) -> None:
        ...
        payload = {
            ...
            "nbf": now - 1200,
            "exp": now + 600,
            "jti": "half-life-token-jti",
        }
        token = pyjwt.encode(payload, self.jwt_mod._CURRENT, algorithm="HS256")

        result_payload, new_token = await self.jwt_mod.decode_token(token, redis_conn=_redis_allowing_tokens())
    ...
    async def test_runtime_env_secret_change_is_respected_without_reload(self) -> None:
        ...
        with patch.dict(
            ...
            payload, new_token = await self.jwt_mod.decode_token(old_token, redis_conn=_redis_allowing_tokens())
