...
class TestResourceReservation:
    def test_overlaps_true(self):
        ...
        r = ResourceReservation(
            ...
            start_at=now, end_at=now + datetime.timedelta(seconds=600),
            priority=80,
        )
    ...
    def test_resource_conflicts(self):
        r = ResourceReservation(
            job_id="j1", node_id="n1",
            start_at=_utcnow(), end_at=_utcnow() + datetime.timedelta(seconds=600),
            priority=80, cpu_cores=4.0,
        )
...
class TestReservationManager:
    ...
    def test_get_node_reservations(self):
        ...
        mgr.create_reservation(
            _make_job(job_id="j1"), node, start_at=now,
            estimated_duration_s=300,
        )
        mgr.create_reservation(
            _make_job(job_id="j2"), node,
...
class TestBackfillEvaluator:
    ...
    def test_cannot_backfill_would_delay_reservation(self):
        ...
        mgr.create_reservation(
            _make_job(job_id="reserved-1", priority=90), node,
